...
export function ActionStats({ actionId }: ActionStatsProps) {
    ...
    const loadData = async () => {
      try {
        ...
        if (targetsData && targetsData.length > 0) {
          const totalProgress = targetsData.reduce((acc, target) => {
            return acc + calculateProgress(target.current_value, target.target_value);
          }, 0) / targetsData.length;

          setStats({
            status: determineStatus(totalProgress),
  ...
  const loadJobStats = async () => {
    try {
      ...
      jobTargets.forEach(target => {
        if (target.target_value) stats.totalTarget += Number(target.target_value);
        if (target.current_value) stats.totalCurrent += Number(target.current_value);
        if (target.women_target) stats.womenTarget += Number(target.women_target);
        if (target.women_current) stats.womenCurrent += Number(target.women_current);
        if (target.youth_target) stats.youthTarget += Number(target.youth_target);
        if (target.youth_current) stats.youthCurrent += Number(target.youth_current);
      });
  ...
  const determineStatus = (progress: number) => {
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-lg font-medium text-gray-900">
              {targets[0]?.action?.intervention?.name} - {targets[0]?.action?.name}
            </h3>
            <p className="text-sm text-gray-500 mt-1">Overall Progress</p>
          </div>
          <div className={`p-3 rounded-full ${
            stats.status === 'completed' ? 'bg-green-100' :
            stats.status === 'at_risk' ? 'bg-red-100' : 'bg-yellow-100'
          }`}>
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <div className="flex-1">
                          <div className="flex justify-between text-xs text-gray-500 mb-1">
                            <span>Women</span>
                            <span>{target.women_current} / {target.women_target}</span>
                          </div>
                    ...
                    {target.youth_target && (
                      <div className="flex items-center space-x-2">
                        ...
                        <div className="flex-1">
                          <div className="flex justify-between text-xs text-gray-500 mb-1">
                            <span>Youth</span>
                            <span>{target.youth_current} / {target.youth_target}</span>
                          </div>
